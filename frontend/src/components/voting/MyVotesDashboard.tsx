...
export const MyVotesDashboard: React.FC<Props> = ({ onClose }) => {
  ...
  const clearVoteHistory = () => {
    if (window.confirm('Are you sure you want to clear your local vote history? This only removes the display cache — your vote is permanently recorded on the blockchain.')) {
      localStorage.removeItem('votelink_votes');
      setVotes([]);
    }
  };
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="flex items-start justify-between mb-8 mv-float" style={{ animationDelay: '0s' }}>
              ...
              <div className="flex items-center gap-2">
                ...
                {votes.length > 0 && (
                  <button
                    onClick={clearVoteHistory}
                    className="mv-btn px-3 py-2 rounded-lg text-xs font-medium text-red-600 hover:text-red-800"
                    style={{ background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(220,38,38,0.2)', backdropFilter: 'blur(8px)' }}
                    title="Clear local vote history cache"
                  >
                    Clear History
                  </button>
                )}
