...
export const Web3VotingModal: React.FC<Props> = ({
  ...
  const [step, setStep] = useState<'wallet' | 'minting' | 'error'>('wallet');
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      for (let i = 0; i < stages.length; i++) {
        ...
        if (i === 2) {
          // ── CRITICAL: capture the actual vote result ──────────────────
          try {
            const result = await onVote();
            voteSucceeded = result === true;
            if (!voteSucceeded) {
              voteFailureMessage =
                'Your vote could not be recorded. You may have already voted in this election.';
            }
          } catch (err) {
            voteSucceeded = false;
            voteFailureMessage =
      ...
      if (!voteSucceeded) {
        // Vote failed — show error state, do NOT call onSuccess
        setVoteError(voteFailureMessage || 'Vote could not be recorded.');
        setStep('error');
        return;
      }

      // Only reach here if vote truly succeeded
      onSuccess({
        ...txData.current,
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto">
        ...
        <div className="min-h-screen flex items-center justify-center p-4">
          ...
          {step === 'wallet' ? (
            ...
            /* ── ERROR STATE — shown when backend rejects the vote ── */
            <div className="bg-white/20 backdrop-blur-xl rounded-3xl shadow-2xl p-8 w-full max-w-md border border-white/30 text-center relative">
              <button
                onClick={onCancel}
                className="absolute top-4 right-4 text-gray-500 hover:text-gray-800 transition-colors"
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-3">Vote Not Recorded</h2>
              ...
              >
                <p className="text-red-700 text-sm leading-relaxed">{voteError}</p>
              </div>
