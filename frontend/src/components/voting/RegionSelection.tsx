...
export const RegionSelection: React.FC<RegionSelectionProps> = ({
  ...
  const convertServiceElectionToElectionInfo = (election: ServiceElection): ElectionInfo => {
    ...
    const isWithinVotingWindow = !Number.isNaN(start.getTime()) && !Number.isNaN(end.getTime()) && now >= start && now <= end;
    const isActiveForVoting = (election.status === 'active' || election.status === 'scheduled') && isWithinVotingWindow;
  ...
  useEffect(() => {
    ...
    const loadAdminElections = (adminElections: ServiceElection[]) => {
      ...
      // Get only elections currently open for voting.
      const liveAdminElections = adminElections.filter(e => {
        const start = new Date(e.startDate);
        const end = new Date(e.endDate);
        const isWithinVotingWindow = !Number.isNaN(start.getTime()) && !Number.isNaN(end.getTime()) && now >= start && now <= end;
        return (e.status === 'active' || e.status === 'scheduled') && isWithinVotingWindow;
      });
  ...
  useEffect(() => {
    ...
    if (selectedState) {
      filtered = filtered.filter(region =>
        region.state === selectedState || region.state === 'All States'
      );
    }
    ...
    if (selectedDistrict) {
      filtered = filtered.filter(region =>
        region.district === selectedDistrict || region.district === 'All Districts'
      );
    }
