    ...
    if (user && !hasVotedInCurrentElection) {
      ...
      try {
        ...
        if (!voteResponse.ok) {
          ...
          // Handle voter ID mismatch
          if (voteData.error === 'VOTER_ID_MISMATCH') {
            throw new Error(voteData.message || 'Voter ID does not match the registered account.');
          }
          // Handle blockchain unavailable
          if (voteData.error === 'BLOCKCHAIN_UNAVAILABLE') {
            throw new Error(voteData.message || 'Blockchain node is not available. Please try again later.');
          }
        ...
        const updatedUser = {
          ...
          votedAt: new Date(),
          voteTransactionHash: voteData.blockchainTxHash || voteReceipt?.transactionHash || undefined,
          voteBlockNumber: voteData.blockchainBlockNumber || voteReceipt?.blockNumber || undefined,
