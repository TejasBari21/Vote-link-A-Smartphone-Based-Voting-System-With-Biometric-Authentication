...
// Marks legacy votes (recorded before blockchain was mandatory) as confirmed
// if they were already recorded on the blockchain but missing the hash.
// New votes always require blockchain — this handles only old data.
async function confirmPendingBlockchainVotes() {
  try {
    ...
    for (const vote of pendingVotes) {
      try {
        // Check if this voter already exists on blockchain (from a partial success)
        const { ethers } = require('ethers');
        const voterHash = ethers.keccak256(ethers.toUtf8Bytes(vote.voterID));
        const { contract } = await blockchainVoteService._getContract();
        const alreadyVoted = await contract.hasVotedCheck(voterHash);

        if (alreadyVoted) {
          await Vote.findByIdAndUpdate(vote._id, { blockchainConfirmed: true });
          console.log(`ℹ️  Vote ${vote._id} already on blockchain — marked confirmed`);
          confirmed++;
        } else {
          // Cannot re-record: plaintext candidateId is no longer stored.
          // These are legacy votes from before blockchain was mandatory.
          console.warn(`⚠️  Vote ${vote._id} cannot be replayed — legacy vote without blockchain recording`);
          failed++;
        }
      } catch (err) {
...
app.get('/api/admin/voting-status/:email', async (req, res) => {
  try {
    ...
    // Get ALL votes for this user (for per-election tracking) — no candidate data exposed
    const votes = await Vote.find({ userEmail: normalizedEmail }).select('electionId votedAt blockchainTxHash blockchainConfirmed blockchainBlockNumber');
...
// Returns blockchain-confirmed votes with transaction details.
// No candidate/party data is exposed — only blockchain metadata.
app.get('/api/blockchain/transactions', async (req, res) => {
  try {
    ...
    const filter = { blockchainConfirmed: true, blockchainTxHash: { $ne: null } };
    ...
    const votes = await Vote.find(filter)
      ...
      .limit(limitNum)
      .select('blockchainTxHash blockchainBlockNumber blockchainConfirmed electionId region.constituency votedAt');

    res.json({
      success: true,
      transactions: votes.map(v => ({
        txHash: v.blockchainTxHash,
        blockNumber: v.blockchainBlockNumber,
        confirmed: v.blockchainConfirmed,
        electionId: v.electionId,
        constituency: v.region?.constituency,
        timestamp: v.votedAt
      })),
      pagination: {
        page: pageNum,
...
// ── Blockchain live stats from smart contract ────────────────────────────
// Queries the blockchain directly for election info and vote counts.
app.get('/api/blockchain/stats', async (req, res) => {
  try {
    ...
    res.status(503).json({
      success: false,
      error: 'Blockchain node is not reachable',
      details: err.message
    });
  }
});
  ...
  try {
    ...
    res.json({
      ...
      transaction: {
        ...
        timestamp: tx.blockNumber ? new Date((await provider.getBlock(tx.blockNumber)).timestamp * 1000).toISOString() : null
      }
    });
  } catch (err) {
    res.status(503).json({
