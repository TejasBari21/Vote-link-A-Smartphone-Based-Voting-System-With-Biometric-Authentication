...
exports.recordVote = async (req, res) => {
  try {
    ...
    // Require a valid voter ID — anonymous votes are not allowed
    if (!voterID || voterID === 'Not Set' || voterID === 'NOT_SET' || !voterID.trim()) {
      return res.status(400).json({
        success: false,
        error: 'VOTER_ID_REQUIRED',
        message: 'A valid Voter ID is required to cast a vote.'
      });
    }

    const normalizedVoterID = voterID.toUpperCase().trim();

    // Find user by voterID first, then by email as fallback
    let user = null;
    user = await User.findOne({ voterID: normalizedVoterID });
    ...
    // ── VOTER ID CONSISTENCY CHECK ────────────────────────────────────────
    // If the user was found by email but has a different voterID on record,
    // block the request — voters cannot switch IDs.
    if (user.voterID && user.voterID !== normalizedVoterID) {
      console.log(`🚫 Voter ID mismatch: account ${user.email} has voterID ${user.voterID} but request sent ${normalizedVoterID}`);
      return res.status(403).json({
        success: false,
        error: 'VOTER_ID_MISMATCH',
        message: 'The Voter ID provided does not match the one registered with this account.'
      });
    }

    // Resolve the effective Voter ID — always prefer the DB record
    const effectiveVoterID = user.voterID || normalizedVoterID;
    ...
    // Blockchain recording via admin key — REQUIRED for vote integrity
    ...
    if (!finalBlockchainTxHash) {
      try {
        ...
        blockchainBlockNumber = bcReceipt.blockNumber;
        blockchainConfirmed = true;
        console.log('✅ Backend blockchain recording succeeded:', finalBlockchainTxHash);
      } catch (bcError) {
        console.error('❌ Blockchain recording failed:', bcError.message);
        return res.status(503).json({
          success: false,
          error: 'BLOCKCHAIN_UNAVAILABLE',
          message: 'Vote could not be recorded on the blockchain. Please ensure the blockchain node is running and try again.'
        });
      }
    }

    // Hash sensitive vote fields — only hashes are stored in MongoDB
    const hashField = (value) => crypto.createHash('sha256').update(String(value)).digest('hex');

    // Record vote - use the effective voter ID
    const vote = await Vote.create({
      voterID: effectiveVoterID,
      userEmail: user.email,
      electionId: electionId || 'default_election',
      candidateIdHash: hashField(candidateId),
      candidateNameHash: hashField(candidateName),
      partyNameHash: hashField(partyName),
      region,
...
exports.getVotingStats = async (req, res) => {
  try {
    ...
    // Recent votes (no candidate/party data exposed — only hashes and blockchain info)
    const recentVotes = await Vote.find()
      .sort({ votedAt: -1 })
      .limit(10)
      .select('voterID region.constituency blockchainTxHash blockchainConfirmed votedAt');
